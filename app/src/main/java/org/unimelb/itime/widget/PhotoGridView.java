...
public class PhotoGridView extends ViewGroup {
    ...
    private Map<PhotoUrl, ImageView> photoMap;
    private int maxnum = Integer.MAX_VALUE;
    private boolean addable = true;
    private OnClickListener addMoreOnClickListener;
    private ItemOnClickListener itemOnClickListener;
    private View addMoreView;

    public OnClickListener getAddMoreOnClickListener() {
        return addMoreOnClickListener;
    }

    public void setAddMoreOnClickListener(OnClickListener addMoreOnClickListener) {
    ...
    @BindingAdapter({"app:editable", "photos", "maxnum"})
    public static void bindingPhoto(PhotoGridView view, boolean editable, List<PhotoUrl> photos, int maxnum){
        view.photos = photos;
        view.removeAllViews();
        for(int i=0;i<photos.size();i++){
            ImageView photo = view.getPhotoView(photos.get(i));
            if(photo!=null){
                photo.setTag(i);
                view.addView(photo);
            }
        ...
        if(editable&&photos.size()< maxnum){
            view.addView(view.getAddMoreView());
        }
    }

    public ImageView getPhotoView(PhotoUrl url){
        if(photoMap.containsKey(url)){
            ImageView view = photoMap.get(url);
            if(view!=null){
                return view;
            }
        }
        ...
        if (!url.getUrl().equals("")){
            Picasso.with(getContext()).load(url.getUrl()).placeholder(R.drawable.icon_details_photo_placeholder)
                    .memoryPolicy(NO_CACHE, NO_STORE)
                    .error(R.drawable.icon_details_photo_placeholder).resize(200,200).centerCrop().into(imageView);
            photoMap.put(url, imageView);
            return imageView;
        }
        ...
        if (!url.getLocalPath().equals("")){
            ...
            if (file.exists()){
                Picasso.with(getContext()).load(new File(url.getLocalPath())).placeholder(R.drawable.icon_details_photo_placeholder)
                        .memoryPolicy(NO_CACHE, NO_STORE)
                        .error(R.drawable.icon_details_photo_placeholder).resize(200,200).centerCrop().into(imageView);
                photoMap.put(url, imageView);
                return imageView;
    ...
    private View getAddMoreView(){
        if(addMoreView==null) {
            ...
            Picasso.with(getContext()).load(R.drawable.icon_add_more_photos)
                    .resize(200, 200).centerCrop().into(imageView);
            addMoreView = imageView;
        }
        return addMoreView;
    }
